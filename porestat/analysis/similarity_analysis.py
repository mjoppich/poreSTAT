...
class SimilarityAnalysis(ParallelPSTInterface):
    ...
    def makeResults(self, parallelResult, oEnvironment, args):
        ...
        for evidence in evidences:
            ...
            """
            for method in avgL2FCByMethod:

            avgL2FC = avgL2FCByMethod[method]
            allConditions = sorted(list(set([x[0] for x in avgL2FC]).union(set([x[1] for x in avgL2FC]))))

            npArray = np.zeroes( (len(allConditions), len(allConditions)) )

            for condPair in avgL2FC:
                idxi = allConditions.index(condPair[0])
                idxj = allConditions.index(condPair[1])

                npArray[idxi, idxj] = avgL2FC[condPair]
                npArray[idxj, idxi] = avgL2FC[condPair]

            print(npArray)

            pickle.dump(npArray, open('/tmp/heatmap.pickle', 'wb'))
    ...
    @classmethod
    def calcLFCAvgDistance(self, cond1, cond2):
        ...
        for ugene in unionGenes:
            ...
            cond1val = 100*cond1[ugene] / (totalCond1)
            cond2val = 100*cond2[ugene] / (totalCond2)

            commonPairs = (cond1val, cond2val)

            dist += math.pow((commonPairs[0]-commonPairs[1]), 2)
            #dist += commonPairs[0] * commonPairs[1]

        rawSimilarity = math.sqrt(dist) / 100
        print(rawSimilarity)

        return 1.0-rawSimilarity
