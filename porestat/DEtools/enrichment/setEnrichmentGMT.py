...
if __name__ == '__main__':
    ...
    parser.add_argument('-g', '--gmt', type=argparse.FileType('r'), required=True, help='set file')
    ...
    availSets = {}
    allSetGenes = set()
    for line in args.gmt:
        line = line.strip().split("\t")
        
        setID = line[0]
        setName = line[1]
        setGenesRaw = set(line[2:])
        
        setGenes = set()
        for y in setGenesRaw:
            for x in y.split(";"):
                if x in sym2approvSym:
                    setGenes.add(sym2approvSym[x])
                else:
                    setGenes.add(x)
        ...
        availSets[setID] = (setName, setGenes)
    ...
    allExts = args.output.name.split(".")
    allExts[-1]="xlsx"
    xlsxOut = ".".join(allExts)
    
    outdf.export(xlsxOut, exType=ExportTYPE.XLSX)