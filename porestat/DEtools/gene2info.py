...
def main(args):
    ...
    with log("Calculating coding region (exonic) length for each gene..."):
        ...
        rt_start = groups.apply(get_rt_start)
        rt_end = groups.apply(get_rt_end)
...
def get_biotype(df):
    ...
    elems = list(set(df[fieldname]))
    
    elems = [x for x in elems if not x is None]
