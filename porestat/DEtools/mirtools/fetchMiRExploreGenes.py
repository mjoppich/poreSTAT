...
class miRGeneGraph:
    ...
    def getTopRegulatedMIRNAS(self, graph, direction, maxNodes=50):
        ...
        for node in graph.nodes():
            ...
            for nn in nodeNN:
                ...
                if not tgtData.get("de_measured", False):
                    continue

                if tgtData.get("adjPval", 1.0) > 0.05:
                    continue
            ...
            totalDivValue = totalDirectionCounter[self.dirToOpposite[direction]]
            totalDirectionCount = totalDirectionCounter[direction]

            ratio = float(directionCount) / float(divValue if divValue > 0 else 1)
            totalRatio = float(totalDirectionCount) / float(totalDivValue if totalDivValue > 0 else 1)

            node2ratio[node] = (node, nodeData["de_measured"], ratio*directionCount, ratio, directionCount , divValue, totalRatio, totalDirectionCount, totalDivValue)


        allNodeScores = [node2ratio[x][2] for x in node2ratio]
        allNodeScoresZScores = stats.zscore(allNodeScores)

        score2zscore = {}
        for i in range(0, len(allNodeScores)):
            score2zscore[allNodeScores[i]] = allNodeScoresZScores[i]

        bestNodes = sorted([x for x in node2ratio], key=lambda x:node2ratio[x][2], reverse=True)

        if len(bestNodes) > maxNodes:
            bestNodes = bestNodes[0:maxNodes]

        retRatio = [("miRNA", "Measured?", "Score", "Measured Ratio", "Measured {} elements".format(direction), "Measured {} elements".format(self.dirToOpposite[direction]),
                     "Total Ratio", "Total {} elements".format(direction), "Total {} elements".format(self.dirToOpposite[direction]), "zscore")]

        for x in bestNodes:

            savedElem = list(node2ratio[x])

            savedElem.append(score2zscore[savedElem[2]])

            retRatio.append(tuple(savedElem))

        return retRatio

    def writeTopRegulatedMIRNA(self, outfilename, upregs, downregs):
        """
        ...
        header = ["elem_id","population_size","success_population","sample_size","success_samples","sample_success_fraction","pval","adj_pval", "direction", "genes"]

        print("\t".join(header), file=outfile)

        #reg elems
        #miRNA, Measured, Score, Ratio, dir_elements, opp_dir_elements, total_ratio, total_dir_elements, total_opp_dir_elements, zscore
        ...
        for idx, elem in enumerate(upregs[1:]+downregs[1:]):
            ...
            success_fraction  = 0
            pval = allpvals[idx]
            adj_pval = allAdjPvals[idx]
            direction = directions[idx]
            genes = []

            print(miRNA, pop_size, success_pop, sample_size, success_sample, success_fraction, pval, adj_pval, direction, ";".join(genes), sep="\t", file=outfile)
    ...
    def saveGraph(self, outpath, outhtml, graph):
        ...
        graphStats["UP regulated miRNAs"] = self.getTopRegulatedMIRNAS(graph, "up", 50)
        graphStats["DOWN regulated miRNAs"] = self.getTopRegulatedMIRNAS(graph, "down", 50)
        ...
        upregMirs = self.getTopRegulatedMIRNAS(graph, "up", -1)
        downregMirs = self.getTopRegulatedMIRNAS(graph, "down", -1)
        self.writeTopRegulatedMIRNA(basename + ".mirs.tsv", upregMirs, downregMirs)
    ...
    def __impute4(self, graph):
        ...
        for node in graph.nodes():
            ...
            print("impute4", node, nnDirDirections, noEvidenceDir, nnWithNoEvidence)
...
if __name__ == '__main__':
    ...
    for didx, detable in enumerate(args.detable):
        ...
        if not REFETCHDATA and os.path.exists("mirnafetch.pickle"):
            with open('mirnafetch.pickle', 'rb') as f:
            ...
            with open('mirnafetch.pickle', 'wb') as f:
                pickle.dump(mirnaHits, f)
                print("Wrote out mirnafetch")

        if not REFETCHDATA and os.path.exists("genefetch.pickle"):
            ...
            with open('genefetch.pickle', 'wb') as f:
